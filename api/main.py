...
async def process_single_company(
    ...
    """Process a single company discovery in the background"""
    ...
    try:
        ...
        logger.info(f"Job {job_id}: Processing {company_name}")
        
        # Simulate API key validation
        await asyncio.sleep(1)
        jobs_storage[job_id]["progress"] = 25
        jobs_storage[job_id]["message"] = "Validating API keys..."
        
        # Simulate discovery process
        await asyncio.sleep(2)
        jobs_storage[job_id]["progress"] = 50
        jobs_storage[job_id]["message"] = "Searching for locations..."
        
        await asyncio.sleep(2)
        jobs_storage[job_id]["progress"] = 75
        jobs_storage[job_id]["message"] = "Processing and validating results..."
        
        # Mock result (replace with actual workflow integration)
        result = {
            "company_name": company_name,
            "company_url": company_url,
            "locations": [
                {
                    "Location_Name": f"{company_name} Headquarters",
                ...
                {
                    "Location_Name": f"{company_name} West Coast Office",
            ...
            "summary": {
                ...
                "url_processed": company_url is not None
            },
            "messages": [
                "Successfully processed company website",
                "Found locations via Google Maps API",
...
async def process_batch_companies(
    ...
    try:
        ...
        jobs_storage[job_id]["status"] = "running"
        jobs_storage[job_id]["message"] = f"Processing batch of {total_companies} companies"
        
        for i, company in enumerate(companies):
            progress = int((i / total_companies) * 90)  # Reserve 10% for final processing
            jobs_storage[job_id]["progress"] = progress
            jobs_storage[job_id]["message"] = f"Processing {company.company_name} ({i+1}/{total_companies})"
            ...
            # Mock result for each company
            company_result = {
                "company_name": company.company_name,
                "company_url": company.company_url,
                "locations": [
                    {
                        "Location_Name": f"{company.company_name} Office",
                        "Street_Address": f"Mock address for {company.company_name}",
                        "City": "Sample City",
                        "Country": "USA",
                        "Data_Source": "mock",
                ...
                "summary": {
                    "total_locations": 1,
                    "sources_used": {"google_maps": 0, "website": 1, "tavily": 0, "sec": 0},
                    "url_processed": company.company_url is not None
                },
                "messages": [f"Processed {company.company_name}"],
                "errors": []
            }
